...
export interface BalanceCategoriesProps {
  chartSeries: number[];
  labels: string[];
  sx?: SxProps;
}

export function BalanceCategories({ chartSeries, labels, sx }: BalanceCategoriesProps): React.JSX.Element {
  const chartOptions = useChartOptions(labels);

  return (
    <Card sx={sx}>
      <CardHeader title="Balance Categories" />
      <CardContent>
        <Stack spacing={2}>
